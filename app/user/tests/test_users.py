...
class UserApiTests(TestCase):
    ...
    def test_register_missing_password(self):
        ...
        response = self.client.post(RESISTER_USER_URL, payload)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertContains(response, "Password may not be blank", None, 400)
    ...
    def test_register_missing_email(self):
        ...
        response = self.client.post(RESISTER_USER_URL, payload)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertContains(response, "Email may not be blank", None, 400)
    ...
    def test_refresh_token_invalid_token(self):
        ...
        payload = {"refresh": refresh_token[:-1]}
        response = self.client.post(REFRESH_TOKEN_URL, payload)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_refresh_token_empty(self):
        ...
        response = self.client.post(REFRESH_TOKEN_URL, payload)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertContains(response, "Refresh may not be blank", None, 400)
    ...
    def test_password_reset_invalid_email(self):
        ...
        create_user(**payload)
        new_payload = {"email": "dummy@pxfld.cz"}
    ...
    def test_password_reset_token_is_empty(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertContains(response, "Token may not be blank", None, 400)
    ...
    def test_password_change_success(self):
        ...
        key = ResetPasswordToken.generate_key()
        user.password_reset_tokens.create(key=key)
        payload = {"token": key, "password": "sgfgfgfd"}
    ...
    def test_password_change_invalid_token(self):
        ...
        key = ResetPasswordToken.generate_key()
        user.password_reset_tokens.create(key=key)
        payload = {"token": key + "1", "password": "sgfgfgfd"}  # Making invalid key
        response = self.client.post(PASSWORD_RESET_CONFIRM_URL, payload)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_password_change_empty_token(self):
        ...
        key = ResetPasswordToken.generate_key()
        user.password_reset_tokens.create(key=key)
        payload = {"token": "", "password": "sgfgfgfd"}
        response = self.client.post(PASSWORD_RESET_CONFIRM_URL, payload)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertContains(response, "Token may not be blank", None, 400)
